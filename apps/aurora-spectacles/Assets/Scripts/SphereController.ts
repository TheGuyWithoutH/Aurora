...
export class SphereController extends BaseScriptComponent {
  ...
  private baseAnimationLayer: AnimationMixerLayer;
  private normalAnimSpeed: number = 0.5; // Default speed
  private thinkingAnimSpeed: number = 2.0; // Faster when thinking
  ...
  onAwake() {
    ...
    if (this.animationMixer) {
      this.baseAnimationLayer = this.animationMixer.getLayer("BaseLayer");
      if (this.baseAnimationLayer) {
        this.normalAnimSpeed = this.baseAnimationLayer.speedRatio;
      }
    }
  ...
  // Set animation to "thinking" mode (faster speed)
  public setThinkingMode(isThinking: boolean) {
    if (!this.baseAnimationLayer) {
      return;
    }

    if (isThinking) {
      // Accelerate animation when waiting for response
      this.baseAnimationLayer.speedRatio = this.thinkingAnimSpeed;
      print("Animation speed: Thinking mode (fast)");
    } else {
      // Return to normal speed
      this.baseAnimationLayer.speedRatio = this.normalAnimSpeed;
      print("Animation speed: Normal mode");
    }
  }
}
