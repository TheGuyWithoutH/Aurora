...
export class CustomAgent extends BaseScriptComponent {
  ...
  // Get or create a device ID (stored in persistent storage)
  private getOrCreateDeviceId(): string {
    // Try to get existing device ID from persistent storage
    const storedDeviceId = global.persistentStorageSystem.general.getString(
      "aurora_device_id"
    );
    ...
    const newDeviceId = "spectacles-" + this.generateHexString(16);
    global.persistentStorageSystem.general.putString(
      "aurora_device_id",
      newDeviceId
    );
