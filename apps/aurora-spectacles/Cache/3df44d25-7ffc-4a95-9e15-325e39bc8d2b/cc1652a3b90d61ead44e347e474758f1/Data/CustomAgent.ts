...
export class CustomAgent extends BaseScriptComponent {
  ...
  private startListening() {
    ...
    asrSettings.onTranscriptionUpdateEvent.add((asrOutput) => {
      ...
      if (asrOutput.isFinal) {
        ...
        // Check if user wants to take a picture
        const lowerText = asrOutput.text.toLowerCase();
        if (
          lowerText.includes("take a picture") ||
          lowerText.includes("take photo") ||
          lowerText.includes("what do you see")
        ) {
          // Capture and analyze image
          this.captureAndSendImage(asrOutput.text);
        } else {
  ...
  public async captureAndSendImage(prompt: string = "What do you see?") {
  ...
  private async sendImageToRpc(base64Image: string, prompt: string) {
    try {
